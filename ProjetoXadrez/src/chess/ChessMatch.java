...
public class ChessMatch {
	...
	public Color getCurrrentPlayer() {
	...
	public ChessPiece performamChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("You can't put yoruself in check");
		}
		
		ChessPiece movedPiece = (ChessPiece)board.piece(target);
		
		//#moviemnto de promoção
		promoted =null;
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece p =(ChessPiece) board.removePeice(source);
		p.increaseMoveCount();
		...
		if(p instanceof King && target.getColumn() == source.getColumn()+2) {
			...
			ChessPiece rook = (ChessPiece)board.removePeice(targetT);
		...
		//en Passant
				if(p instanceof Pawn) {
					if(source.getColumn() != target.getColumn()&& capturedPiece== null) {
						...
						capturedPiece = board.removePeice(pawnPosition);
						capturedPieces.add(capturedPiece);
						piecesOnTheBoard.remove(capturedPiece);
					}
				}
		...
		ChessPiece p = (ChessPiece)board.removePeice(target);
		p.decreaseMoveCount();
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				ChessPiece pawn = (ChessPiece)board.removePeice(target);
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
